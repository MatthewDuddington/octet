...
namespace octet {
  ...
  class sprite {
    ...
    void ChangeUVPosition( float lower_left_x
      ,                    float lower_left_y )
    {
  ...
  public:
    ...
    const mat4t& ModelToWorld() { return modelToWorld; }
    ...
    // Added:
    // TODO Replace with an 'Animation' class object which contains a series of steps to carry out enabling compound behaviours.
    int Animate(AnimationName animation_name)
    {  
      ...
      ChangeUVPosition( sprite_size.x() * (1 + animation_frame_counter)
                      , sprite_size.y() * (1 + animation_frame_counter) );
    }
